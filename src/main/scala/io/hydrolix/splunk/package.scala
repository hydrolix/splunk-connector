...
package object splunk {
   ...
   */
  def readChunk(stdin: InputStream): (ChunkedRequestMetadata, Option[(Int, InputStream)]) = {
    val readF = Future {
      val pb = new PushbackInputStream(stdin, 128)

      val headerBuf = Array.ofDim[Byte](128)

      val len = stdin.read(headerBuf)
      if (len == -1) sys.error("No data available from stdin")
